...
10. **`const fn violation_key` stays exhaustive.** Adding the `SignatureDriftWithinSide` variant requires a matching arm with rank 9 (after `Context` rank 8). This is a compile blocker if missed; R4-6 prescription names it.
