...
status: Draft (revised round 1 — 4-lens council REQUEST CHANGES folded 2026-05-26; pending round 2)
date: 2026-05-26
...
The `report` subcommand sits alongside `check` in `application/src/main.rs`'s `Command` enum. `--verb-coverage` is a flag (not a positional) so future report kinds (e.g. `--tier-histogram`-only) can be added without breaking the wire form.
...
Implementation: extend the current `match item` in `visit_top_level_item` (`adapters/rust/src/lib.rs:114-126`) to include `syn::Item::Fn(f)` for the new walk. No new syn features required (current `syn = "2"` workspace pin with `["full", "parsing", "visit", "extra-traits"]` already supports `ItemFn.vis` + `ItemFn.sig.ident`).

**`MarkdownReader::extract_invariant_annotations` — `adapters/markdown/src/lib.rs`.** Inherent method (NOT on a new port trait — the markdown reader is the only adapter that parses markdown). **Parser model (per rust-systems §5.4 blocker 2):** independent `Parser::new(source)` per file — a fresh `pulldown-cmark` parser instance, not a shared event stream with the existing concept walk. The markdown source `&str` is already in memory (file-load cost paid once); the second parser allocation is the only cost. This is the RFC-001 §3.6 precedent (each marker-driven extraction owns its own parse) applied to invariant annotations.

The extractor scans `## Operational invariants` sub-sections (recognized by H4 heading `#### Operational invariants`) and parses each bullet for two bracketed annotation shapes:
...
- `pub struct HomonymRecord { name: String, contexts: Vec<HomonymAppearance> }` where `pub struct HomonymAppearance { context_name: String, sanctioned_by_pattern: Option<ContextPattern> }`. Per DDD §5.2 finding 1: `sanctioned_by_pattern` is derived from the `CheckInput.contexts` `ContextImport`/`ContextExport` declarations — `Some(PublishedLanguage)` or `Some(SharedKernel)` means the cross-context appearance is doctrine-sanctioned (no council attention warranted); `Some(Conformist)` / `Some(CustomerSupplier)` or `None` (undeclared) means the appearance is a potential split-brain (warrants council review). The NDJSON example in §3.4 reflects this enrichment.
...
{"schema_version":"2","record":"homonym","name":"Severity","contexts":[{"context":"validators","sanctioned_by_pattern":null},{"context":"review","sanctioned_by_pattern":null}]}
{"schema_version":"2","record":"homonym","name":"Graph","contexts":[{"context":"equivalence","sanctioned_by_pattern":"PublishedLanguage"},{"context":"reading","sanctioned_by_pattern":"PublishedLanguage"}]}
```
...
- The decisive precedent is `domain::diff` (`domain/src/diff.rs`, re-exported `domain/src/lib.rs:17`), which takes pre-materialized `CheckInput + Graph` and returns `Vec<Violation>`. It has been in `domain` since the first equivalence check ratified, and the clean-arch lens has never contested that placement. `report_verb_coverage` has structurally identical shape: pre-materialized inputs in, pure records out, no I/O, no reader calls.
...
Round 2 review: each lens re-reads §3 (revised) + §4 (revised, +Invariants 6/7/8) + §5.5 (override). The ask is:
- Clean-arch: confirm round-1 findings 1, 3, 4 are RESOLVED; ratify or reject the author override on finding 2.
- DDD: confirm round-1 findings 1-4 are RESOLVED; RATIFY.
...
- Rust-systems: confirm all three BLOCKERS RESOLVED; RATIFY.
...
**Scope:** the new `VerbReader` port in `ports/src/lib.rs`; the new domain types (`PubFnDecl`, `InvariantAnnotation`, `VerbCoverageRecord`, `TierHistogramRecord`, `TierKind` with `#[non_exhaustive]`, `HomonymRecord`, `HomonymAppearance`, `ReportOutput`); the pure `report_verb_coverage` function in `domain`; `RustReader: VerbReader` impl; `MarkdownReader::extract_invariant_annotations` inherent method.

**Tests** (minimum prescribed by round 1; round 2 may extend):
- Unit: assertions on `report_verb_coverage` over crafted inputs covering (a) verb-coverage with `None` context; (b) tier histogram with all four `TierKind` values; (c) homonym with sanctioned vs unsanctioned appearances.
